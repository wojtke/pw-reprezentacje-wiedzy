...
public sealed class LanguageCoverageTests
{
    ...
    [Fact]
    public void Observable_After_And_After_Assertions_Validate_Models_Differently()
    {
        // Oba zdania są warunkami poprawności modelu (M2) nad całym Σ₀, nie filtrami.
        // "after" = warunek konieczny (każdy stan początkowy, każdy ślad kończy się celem);
        // "observable after" = warunek możliwy (istnieje stan początkowy i ślad z celem).
        var necessaryDomain = """
            fluents p, q
            actions a
            initially not q
        ...
        var necessary = TestData.Solve(necessaryDomain, "necessary q after a");
        var observable = TestData.Solve(observableDomain, "necessary q after a");

        // "after": stan {not p, not q} łamie warunek konieczny -> dziedzina sprzeczna.
        Assert.False(necessary.Ok);
        Assert.Equal(0, necessary.Sigma0Count);

        // "observable after": istnieje wspierający stan {p, not q} -> dziedzina spójna,
        // a Σ₀ nie jest okrojone (oba stany pozostają).
        Assert.True(observable.Ok, observable.Error);
        Assert.Equal(2, observable.Sigma0Count);
    }
