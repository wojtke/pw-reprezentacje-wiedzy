...
public sealed class LanguageCoverageTests
{
    ...
    [InlineData("r ∨ q", true)]
    [InlineData("p -> q", true)]
    [InlineData("q -> r", false)]
    [InlineData("p <-> q", true)]
    [InlineData("p <-> r", false)]
    [InlineData("(p and q) or r", true)]
    [InlineData("p and (q or r)", true)]
    public void FormulaParser_Covers_All_Logical_Words_And_Symbols(string formulaText, bool expected)
    {
    ...
    [InlineData("possibly executable after ε", Quantifier.Possibly, QueryKind.Executable, 0)]
    ...
    public void ProcessParser_Covers_Empty_Sequential_Comma_And_Composite_Syntax()
    {
        ...
        Assert.Empty(ProcessParser.Parse("ε").Steps);
    ...
    public void EndToEnd_Covers_Causes_Releases_Impossible_Always_Noninertial_Initially_And_Composite_Process()
    {
        var domain = """
            ...
            initially !key and !door_open and alarm and power
            always door_open -> key
            always !power -> !safe
            noninertial safe
            find_key causes key if true
            open_door causes door_open if key
            impossible open_door if !key
        ...
        var full = TestData.Solve(domain, "necessary !alarm after find_key; open_door; reset_alarm");
        var releasedPossible = TestData.Solve(domain, "possibly !safe after cut_power");
        var releasedNecessary = TestData.Solve(domain, "necessary !safe after cut_power");
    ...
    [Fact]
    public void Conditional_Cause_Regression_Is_Covered_At_Query_Level_With_Positive_And_Negative_Oracle()
    {
        var domain = """
            fluents p, q
            actions action
        ...
        Assert.True(necessary.Ok, necessary.Error);
        Assert.True(possibly.Answer);
        Assert.False(necessary.Answer);
        Assert.Contains("[0] start -> {p, ¬q}", possibly.Trace);
        Assert.Contains("[1] action -> {p, q}", possibly.Trace);
        Assert.DoesNotContain("[1] action -> {p, ¬q}", possibly.Trace);
    }
}
